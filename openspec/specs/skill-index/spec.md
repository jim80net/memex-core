...
`parseFrontmatter(content)` SHALL parse content wrapped in `---` frontmatter delimiters and return `{ meta, body }`, extracting `name`, `description`, `type`, `queries`, `keywords`, `paths`, `hooks`, `one-liner`, `boost`, and lifecycle `status` (`active | retired`). Folded and literal description block scalars (`>-`, `>`, `|-`, and `|`) SHALL be normalized to one search-safe line rather than exposing the scalar indicator. The returned `body` SHALL contain the content after the closing delimiter. If frontmatter delimiters are absent, the function SHALL return `{ meta: {}, body: content }` without modification.
