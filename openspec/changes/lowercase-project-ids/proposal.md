...
- **Migration runs only inside `syncPull`**, at two positions: no-remote local-only path, and after a successful rebase/merge. It deliberately does NOT run on the no-commits fresh-repo path — writing a v2 marker before the remote has been fetched would falsely mark legacy v1 content as migrated on the clone-fallback path. Never on stale pre-fetch state, so two devices upgrading concurrently cannot create divergent migration commits.
